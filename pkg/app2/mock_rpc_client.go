...
func (_m *MockRPCClient) Accept(lisID uint16) (uint16, network.Addr, error) {
	ret := _m.Called(lisID)
...
func (_m *MockRPCClient) Dial(remote network.Addr) (uint16, routing.Port, error) {
	...
	var r1 routing.Port
	if rf, ok := ret.Get(1).(func(network.Addr) routing.Port); ok {
		r1 = rf(remote)
	} else {
	...
	var r2 error
	if rf, ok := ret.Get(2).(func(network.Addr) error); ok {
		r2 = rf(remote)
	} else {
		r2 = ret.Error(2)
	}
...
func (_m *MockRPCClient) Listen(local network.Addr) (uint16, error) {
	ret := _m.Called(local)
