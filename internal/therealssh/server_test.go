...
func TestServerOpenChannel(t *testing.T) {
	...
	go func() {
		errCh <- s.OpenChannel(&app.LoopAddr{PubKey: cipher.PubKey{}, Port: Port}, 4, in)
	}()
	...
	go func() {
		errCh <- s.OpenChannel(&app.LoopAddr{PubKey: pk, Port: Port}, 4, in)
	}()
...
func TestServerHandleRequest(t *testing.T) {
	...
	in, out := net.Pipe()
	ch := OpenChannel(4, &app.LoopAddr{PubKey: pk, Port: Port}, in)
	s.chans.add(ch)
